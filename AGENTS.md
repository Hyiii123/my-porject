...
All application runtime services, databases, Redis, and Nacos run exclusively on the remote cloud server (Alibaba Cloud ECS `47.121.31.17`). Local environment is strictly reserved for:
- Code editing, diff review, and version control (`git`).
- Static offline compilation / packaging (`mvn compile / package`, `npm run build`).
- Remote CLI operations and verification tests against the server (Workbench CLI, smoke test targeting `http://47.121.31.17:8080`).

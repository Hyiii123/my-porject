...
const getSeasonsData = () => {
  getSeasons({season:0, pageNo: 1, pageSize: 10})
    .then((res) => {
      if (res.code == 200 ){
        seasonsData.value = normalizeBoard(res.data)
      } else {
        ElMessage({
          message: res.msg,
          type: 'error'
...
const getSignRecordsHandle = async () => {
  await getTodayPoints()
    .then((res) => {
      if (res.code == 200 ){
        const data = res.data || {}
        access.value.find(item => item.value === 2).points = Number(data.todayPoints || 0)
        toadyPointsData.value = data
      } else {
        ElMessage({
          message: res.msg,
          type: 'error'
...
const normalizeBoard = (data) => {
  const rows = Array.isArray(data) ? data : (data?.boardList || data?.list || [])
  const currentUserId = Number(userStore.getUserInfo?.id || userStore.getUserInfo?.userId || 0)
  const current = rows.find(item => Number(item.userId) === currentUserId)
  return {
    rank: current?.rank || data?.rank || '--',
    points: current?.points || data?.points || 0,
    boardList: rows.map(item => ({
      name: item.name || item.userName || item.nickname || '学习者',

    ...
    <div class="calendarCont">
      <div class="day" v-for="item in calendarData">
          <div class="taday" v-if="item.date==currentDay">
            ...
            <span v-else-if="item.isRecords == 99" style=" cursor: auto;">未开始</span>
...
function getMounthDay(day){
  ...
  for(let i = 0; i < calendarDay;i++){
    ...
    arr.push({
      ...
      isRecords: signData.value.includes(date) ? 1 : (moment(date).isAfter(moment(day), 'day') ? 99 : 0) // 0是未打卡 1是已经打卡 99是未开始
    })
  }
  ...
  await getSignRecords()
    .then((res) => {
      if (res.code == 200 ){
        ...
        signData.value = records.map(item => item.date || item.signDate).filter(Boolean)
        // 日历数据处理

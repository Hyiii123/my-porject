      ...
      <div class="tableBox">
        ...
        >
          ...
          <el-table-column prop="size" label="大小（MB）" min-width="120">
            <template #default="scope">
              {{ formatSize(scope.row.sizeBytes ?? scope.row.size) }}
            </template>
          </el-table-column>
          <el-table-column prop="duration" label="视频时长" min-width="120">
            <template #default="scope">
              {{ formatSeconds(scope.row.durationSeconds ?? scope.row.mediaDuration ?? scope.row.duration) }}
            </template>
          </el-table-column>

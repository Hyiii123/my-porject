                  ...
                  <div class="con">
                    ...
                    <div class="videoName">
                      <div style="display:flex;justify-content: center;">
                        <span @click="handleSeeVideo(val.mediaId)">{{ val.mediaName ? ellipsis(val.mediaName,10)+'.mp4' : '' }}</span>
                      </div>
                    </div>
...
const readOnly = true; //详情页没有保存入口，只展示媒资和试看状态
const mediaId = ref(""); //视频id
const title = ref("视频预览");
const preview = ref(null)
const dialogFormVisible = ref(false) //弹层隐藏显示

watch(
  () => props.courseVideoData,
  (value) => {
    itemData.value = Array.isArray(value) ? value : [];
  },

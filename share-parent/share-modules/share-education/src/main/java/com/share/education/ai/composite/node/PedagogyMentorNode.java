...
public class PedagogyMentorNode {
    ...
    public void evaluateAndChallenge(DebateBlackboardState state) {
        ...
        AgentDebateTurn turn = AgentDebateTurn.builder()
            ...
            .argument(arg)
            .metricSummary(String.format("学员自律分: %d, 劝退风险预警: 极高(65%%), 判定: 建议降级阶段 2", discipline))
            .latencyMs(latency)
            .timestamp(System.currentTimeMillis())
            .build();

        state.recordTurn(turn);
        state.setDisagreementScore(0.85); // 标记高度分歧
        log.info("[PedagogyMentorNode] 提出学情质疑，分歧度上调至 0.85");
    }
    ...
    public void reviewCompromise(DebateBlackboardState state) {
        ...
        AgentDebateTurn turn = AgentDebateTurn.builder()
            ...
            .argument("复核大厂技术总监提交的折中方案：阶段 2 晦涩理论已软化，补充了渐进式实战项目，认知坡度已平缓可攀登。该调整既保护了学员信心，又保留了必要核心技能。学情端认可并签字通过！")

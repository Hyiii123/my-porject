...
public class FeeRuleController extends BaseController {
    ...
    @Autowired
    private IFeeRuleService iFeeRuleService;

    @GetMapping("/list")
    public TableDataInfo list(FeeRule feeRule){
        startPage();
        List<FeeRule> feeRules = iFeeRuleService.selectFeeRuleList(feeRule);
        return getDataTable(feeRules);
    }
    /**
     * 获取费用规则详细信息
     */
    ...
    {
        return success(iFeeRuleService.getById(id));
    }
    ...
    {
        return toAjax(iFeeRuleService.save(feeRule));
    }
    ...
    {
        return toAjax(iFeeRuleService.removeBatchByIds(Arrays.asList(ids)));
    }
    ...
    {
        return success(iFeeRuleService.getALLFeeRuleList());
    }

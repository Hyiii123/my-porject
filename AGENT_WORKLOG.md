...
### 2026-09-12 05:50:00 - 用户端首页加载性能与图片渲染彻底优化：SWR 客户端预热 + 请求拓扑解耦 + 去除大表扫描与延迟加载
...
* **核心成果**：
  ...
  2. **前端架构 SWR (Stale-While-Revalidate) 本地预热治理**：
  ...
  4. **图片渲染体验全面升级**：
     - 针对首屏主推荐卡片移除 `loading="lazy"`，并注入 `fetchpriority="high"`，结合 Nginx 静态资源 7 天 `Cache-Control: public, max-age=604800, immutable` 浏览器强缓存，消除图片白块与闪烁现象；
  5. **定向验证与平滑发布**：
     - 本地完成编译打包 `frontends/portal`（无警告报错）；
     - 静态产物同步部署至云端 `tianji-portal-ui` 容器并平滑热重载 Nginx；定向接口验证全链路 200 OK，响应耗时均在 10ms~200ms 内。

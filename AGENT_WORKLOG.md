...
5. **【本地禁启准则】严禁在本地环境中启动微服务或中间件**
   ...
   * **原因**：项目微服务集群已全面统一部署于阿里云 ECS 线上服务器（`47.121.26.136`），所有数据状态、Nacos 配置中心、Redis 缓存与数据库均在线上闭环。在本地拉起本地服务不仅会争抢本地系统端口、造成端口冲突，还会产生因本地与云端配置不一致带来的“脏调用”与环境割裂。
...
| **ECS 公网 IP** | `47.121.26.136` | 线上运行地址（动态公网，当前已切换至 `47.121.26.136`） |
...
| **前端 - 学生端** | `http://47.121.26.136:18081` | 容器 `zhiwen-portal-ui`，对应源码 `frontends/portal` |
| **前端 - 业务管理端** | `http://47.121.26.136:18082` | 容器 `zhiwen-business-admin-ui`，对应源码 `frontends/business-admin` |
| **前端 - 基础管理端** | `http://47.121.26.136:18080` | 容器 `zhiwen-ruoyi-ui`，对应源码 `share-ui` |
| **API 网关 Gateway** | `http://47.121.26.136:8080` | 容器 `zhiwen-gateway`，统一接口入口 |
| **推荐算法微服务** | `http://47.121.26.136:15000` | 容器 `zhiwen-recommend`，内部端口 `5000`，`DRAG-KP4SR` 语义桥接引擎 |
...
| **28** | **Spring 6.1 RestTemplate 发送 Map 导致 Python http.server 报 400 Empty request body** | Spring Boot 3.2 (Spring 6.1) 中 `RestTemplate` 使用默认 `SimpleClientHttpRequestFactory` 序列化 `Map<String, Object>` 时默认采用 `Transfer-Encoding: chunked` 流式传输且不附带 `Content-Length` 报头；而 Python 推荐模型服务（`server.py` 基于标准库 `http.server`）严苛校验 `int(headers.get('Content-Length', 0)) <= 0`，导致合法请求被 Python 识别为空包并拦截。 | 在 Java 端调用远程算法接口前，通过 `objectMapper.writeValueAsString(payload)` 明确序列化为字节数组，显式配置 `headers.setContentLength(bodyBytes.length)` 并以 `HttpEntity<byte[]>` 发出，确保 Python 端 100% 精准读取完整 JSON 载荷。 |
...
| **33** | **多智能体推荐子系统链路缺陷群（TimSort破坏Contract/先修倒置误判/微调参数失配/布鲁姆元数据脱落/指标漏记）** | 1. `PathPlanningAgent` 直接在 `Comparator` 中混杂非传递性的图谱先修依赖与课程难度，破坏 Java TimSort 传递性契约，潜在抛出 `IllegalArgumentException: Comparison method violates its general contract!`；<br>2. `PathCriticAgent` 在核查先修倒置时使用子串双向模糊包含，导致将后续阶段的高级微服务/调优课程误判为前序基础课的未满足前置，虚扣 18 分并误触反思回路；<br>3. 前端人机微调传参 `excludeCourseIds` 与后端接收键 `excludedCourseIds` 大小写/拼写失配，且 `skipBasicPhase` 虽传但后端未实际执行阶段跳过；<br>4. `PersonalizedRecommendVO` 及业务视图脱落 `bloomLevel`、`bloomName`、`isCapstone`、`evidencePaths` 导致前端主页与 HUD 卡片角标空白；<br>5. 前端大屏访问 `criticReport` 不存在字段导致拓扑合法性恒为红字报错；<br>6. SSE 推演流未接入 `evalService`，导致流式思考未纳入在线指标大屏统计。 | 1. 采用标准 **Kahn DAG 拓扑排序算法**（基于入度表与优先队列 Tie-Breaking），数学上严格保证无环拓扑与 Transitivity 契约；<br>2. 审判 Agent 增加高阶技术关键词与难度梯度排他过滤，杜绝后续进阶课反向充当先修基石的误判；<br>3. 双向兼容 `excludeCourseIds` / `excludedCourseIds`，并落地 `skipBasicPhase` 跳过第一阶段；<br>4. 在 VO 模型与 `EducationService` 中补全 Bloom 与 Capstone 标签链路，前端增加多别名容错绑定；<br>5. `CriticReport` 补充兼容 Getter 与属性；<br>6. 在 SSE 推演完成处构造 `AgentWorkflowContext` 并同步上报度量大屏。 |
| **34** | **49 项全系统漏洞与缺陷治理总结避坑（Workbench 链式调用断流/课程金额伪智能乘百/首小节媒资签名延迟）** | 1. `workbench upload` 不支持在单个命令字符串中用 `&&` 链式调用多个 upload，多文件上传会被底层断开连接；<br>2. 数据库中课程价格全站统一为整数分（cents），部分前端组件试图以 `price < 1000 => price * 100` 自动换算，导致低价课程（如 9.9 元=990分）被错误放大 100 倍至 990 元；<br>3. 学习页进入时仅绑定了章节目录而未调用 `getMediasSignature`，导致首屏直接展示“无播放媒资”黑屏。 | 1. 多个文件上传必须打包为 `.tar.gz` 归档后单次上传并在远端解压，或严格逐个执行 workbench 命令；<br>2. 严格废除前端对价格小于 1000 的乘 100“伪智能”，全站统一以分存储、模板 `/ 100` 格式化；<br>3. 页面初始化小节选中后，立即主动拉取该小节媒资签名赋给 `mediaUrl`。 |
| **39** | **前端课程中心 404/登录自销毁重载/管理端与登录页 IAIC 统一重塑（路由未注册与API脱落/登录成功自动注销/管理端视觉风格孤立）** | 1. 课程中心路由 `/classList` 虽有页面文件但未在 `base.js` 中注册，且 `class.js` 漏导 `getFreeClassList` 导致打包中断并报 404；<br>2. `permission.js` 在访问 `/login` 时对已登录用户直接调用 `logout()`，结合 `LoginPass.vue` 不当的 `location.reload()` 导致登录瞬间被自清退回登录页；<br>3. 登录页与管理端沿用旧版黑灰 SaaS 风格，与主站 IAIC 科技蓝完全割裂。 | 1. 在 `base.js` 中补齐 `/classList` 路由并补全 `getFreeClassList` API，同时加入 `PUBLIC_PATHS` 免登访问；<br>2. 修复 `permission.js`：已登录用户访问 `/login` 自动重定向至主页，杜绝误注销；修复 `LoginPass.vue` 采用 `router.push` 平滑跳转；<br>3. 登录页 1:1 融入 IAIC 蓝图网格粒子与玻璃拟态卡片，业务管理端全量导入 IAIC 科技蓝 Token 与 Deep Navy 侧边栏及 Cyan 激活高光。 |
| **38** | **全系统 105 项深度缺陷审计与 IAIC 逆向设计移植（IDOR 越权/0元购绕过/优惠券自动穿透/HikariCP 长事务/IAIC 1:1 前端重塑）** | 1. 访客未登录时 `EduUtils.currentUserId()` 伪装为管理员 1L 导致全站 IDOR 越权；<br>2. 下单接口信任请求体 `isInternalSeckill` 导致 0 元购任意付费课程；<br>3. 传入任意 couponId 在 else 分支自动新建直接抵扣实现无限次套利；<br>4. 面试提交作答在 `@Transactional` 中包含外部大模型 HTTP 调用霸占数据库连接；<br>5. 题库抽取在空题目时 `.get(0)` 越界与重复出题；<br>6. 全局异常处理在参数校验异常时缺少字段判断引发 NPE；<br>7. 原门户主页为传统白底，缺少顶尖产学研科技感。 | 1. `currentUserId()` 访客返回 null，增删改显式调用 `requireCurrentUserId()` 阻断未登录越权；<br>2. `placeOrder` 强制从课程 Snapshot 取真实价格，彻底剥离外部秒杀参数；<br>3. 严格校验用户持有未使用的真实优惠券，删除自动新建穿透逻辑；<br>4. 剥离面试事务边界，原子落库后外部异步调用大模型与向量检定；<br>5. 题库抽取增加空列表安全回退至标准问题；<br>6. 校验异常增加多重非空与字段错误判空；<br>7. 逆向 `https://iaic-universities.cqaip.cn/home` 提取全套 `:root` Design Tokens，1:1 重构导航扫描线、Hero 粒子脉冲发光背景、悬浮公告栏、直角科技卡片与 Deep Navy 页脚。 |
| **37** | **多智能体流式推演连接泄露与度量重启清空缺陷（SSE 无阻断执行/Redis度量脱水/阶段过载与空白阶段/自定义自然语言诉求脱节）** | 1. 客户端关闭 SSE 连接后后台线程仍继续调用高耗时大模型造成资源泄露与额度浪费；<br>2. 微服务重启后内存度量样本与总推演计数归零；<br>3. 课程在 4 阶段间分配时存在极端空阶段或单阶段 >6 门过载；<br>4. 前端仅支持下拉选择固定岗位，无法接受学员自然语言自定义目标输入。 | 1. `streamWorkflow` 在各 Agent 关键步进前全面注入 `isCompleted.get()` 快速退出防护；<br>2. `AgentEvaluationService` 深度集成 Redis 持久化，启动时自动恢复历史样本与运行计数；<br>3. `PathPlanningAgent` 引入 `balanceStageCapacities` 动态阶段容量自适应均衡器；<br>4. 全链路打通 `query` 自然语言参数，前端 HUD 增加自定义诉求输入框并联动意图分发器实体提取。 |
| **36** | **多智能体协同导学系统六大缺陷治理（Actor-Critic 指令脱节/二审无条件通过/第三方大模型单并发骤降/保真度分词漏判/快捷路由假指标污染/动态公网 IP 漂移）** | 1. `IndustryArchitectNode` 折中反思未读取 `refinementDirectives`，仅固定应用单阶段减 5 小时补丁，缺乏针对性重排；<br>2. 图执行引擎二审未通过依然无条件盖章共识，缺少自适应稳定化兜底；<br>3. 第三方大模型客户端 `tryLock()` 无等待直接退化为静态规则，多智能体博弈或并发时丧失 LLM 动态特性；<br>4. 推荐理由保真度纯字面全包含匹配对分词复述产生假阴性漏判；<br>5. 快捷直通通道构造 100 分假报告污染真实 DAG 评测看板；<br>6. ECS 动态公网 IP 漂移至 `47.121.26.136`。 | 1. 总监节点完整读取修正指令集，联动 `PathPlanningAgent` 执行全局 DAG 拓扑重排；<br>2. 图执行引擎引入二审自适应应急稳定化与条件分支；<br>3. `ThirdPartyAiClient` 引入 2500ms 平滑排队与 15 分钟 TTL 多智能体语义缓存；<br>4. `calculateFaithfulness` 升级分词片段与实体核心词关联判定；<br>5. 快捷通道与真实路径规划质量度量池实现严格分流；<br>6. 依照 Rule 7 实施全站公网 IP 校验与全量同步。 |
| **35** | **全系统 56 项大模型幻觉/死锁/状态机缺陷群（线程池自身嵌套饥饿死锁/长事务跨LLM/SSE多行解析/PyTorch全padding掩码NaN/洋葱追问跳维/代码沙箱回填漏计/简历评分不归一/进度联动）** | 1. `MultiAgentRecommendOrchestrator` 内层向同一 `agentThreadPool` 提交画像构建子任务并 `.join()` 导致饥饿死锁；<br>2. 客服接口 `@Transactional` 包裹长耗时外部大模型请求抽干 HikariCP 连接池；<br>3. 前端 SSE 按单换行分割破坏多行 JSON 数据包导致卡死；<br>4. 推荐模型冷启动空序列触发 Softmax 全 True 掩码输出 `NaN`，且 T5 掩码使用 `finfo.min` 叠加产生溢出；<br>5. 剥洋葱追问以轮次数切维度导致跳过中间深度，且代码手撕交卷未回填当前轮次导致总分误判；<br>6. 简历大模型打分与五维启发式重算不一致导致和不等于总分；<br>7. 课程学习进度保存接口返回局部小节导致总览进度归零且未联动学习计划。 | 1. 消除线程池嵌套提交，在主调度线程中运行；<br>2. 剥离客服事务边界，事务内仅落库，外部异步执行大模型与向量检索；<br>3. 重构前端 SSE 缓冲：严格以双换行 `\n\n` 分包并汇聚所有 `data:` 行后再解析；<br>4. PyTorch 掩码校验 `all_padded` 并将首位设为 False 防 `NaN`，T5 掩码使用常量 `-1e4`；<br>5. 面试考题依据已考维度计数切换，提交代码时实时回填轮次并记录沙箱成绩，总评审 Prompt 注入完整代码；<br>6. 简历明细采用动态权重归一化分配，确保各子项严格和等于 `matchScore`；<br>7. `saveLearning` 显式返回课程全局 `summary` 并联动更新 `EduLearningPlan`。 |
| **39** | **前端课程中心 404/登录自销毁重载/管理端与登录页 IAIC 统一重塑（路由未注册与API脱落/登录成功自动注销/管理端视觉风格孤立）** | 1. 课程中心路由 `/classList` 虽有页面文件但未在 `base.js` 中注册，且 `class.js` 漏导 `getFreeClassList` 导致打包中断并报 404；<br>2. `permission.js` 在访问 `/login` 时对已登录用户直接调用 `logout()`，结合 `LoginPass.vue` 不当的 `location.reload()` 导致登录瞬间被自清退回登录页；<br>3. 登录页与管理端沿用旧版黑灰 SaaS 风格，与主站 IAIC 科技蓝完全割裂。 | 1. 在 `base.js` 中补齐 `/classList` 路由并补全 `getFreeClassList` API，同时加入 `PUBLIC_PATHS` 免登访问；<br>2. 修复 `permission.js`：已登录用户访问 `/login` 自动重定向至主页，杜绝误注销；修复 `LoginPass.vue` 采用 `router.push` 平滑跳转；<br>3. 登录页 1:1 融入 IAIC 蓝图网格粒子与玻璃拟态卡片，业务管理端全量导入 IAIC 科技蓝 Token 与 Deep Navy 侧边栏及 Cyan 激活高光。 |
| **38** | **全系统 105 项深度缺陷审计与 IAIC 逆向设计移植（IDOR 越权/0元购绕过/优惠券自动穿透/HikariCP 长事务/IAIC 1:1 前端重塑）** | 1. 访客未登录时 `EduUtils.currentUserId()` 伪装为管理员 1L 导致全站 IDOR 越权；<br>2. 下单接口信任请求体 `isInternalSeckill` 导致 0 元购任意付费课程；<br>3. 传入任意 couponId 在 else 分支自动新建直接抵扣实现无限次套利；<br>4. 面试提交作答在 `@Transactional` 中包含外部大模型 HTTP 调用霸占数据库连接；<br>5. 题库抽取在空题目时 `.get(0)` 越界与重复出题；<br>6. 全局异常处理在参数校验异常时缺少字段判断引发 NPE；<br>7. 原门户主页为传统白底，缺少顶尖产学研科技感。 | 1. `currentUserId()` 访客返回 null，增删改显式调用 `requireCurrentUserId()` 阻断未登录越权；<br>2. `placeOrder` 强制从课程 Snapshot 取真实价格，彻底剥离外部秒杀参数；<br>3. 严格校验用户持有未使用的真实优惠券，删除自动新建穿透逻辑；<br>4. 剥离面试事务边界，原子落库后外部异步调用大模型与向量检定；<br>5. 题库抽取增加空列表安全回退至标准问题；<br>6. 校验异常增加多重非空与字段错误判空；<br>7. 逆向 `https://iaic-universities.cqaip.cn/home` 提取全套 `:root` Design Tokens，1:1 重构导航扫描线、Hero 粒子脉冲发光背景、悬浮公告栏、直角科技卡片与 Deep Navy 页脚。 |
| **37** | **多智能体流式推演连接泄露与度量重启清空缺陷（SSE 无阻断执行/Redis度量脱水/阶段过载与空白阶段/自定义自然语言诉求脱节）** | 1. 客户端关闭 SSE 连接后后台线程仍继续调用高耗时大模型造成资源泄露与额度浪费；<br>2. 微服务重启后内存度量样本与总推演计数归零；<br>3. 课程在 4 阶段间分配时存在极端空阶段或单阶段 >6 门过载；<br>4. 前端仅支持下拉选择固定岗位，无法接受学员自然语言自定义目标输入。 | 1. `streamWorkflow` 在各 Agent 关键步进前全面注入 `isCompleted.get()` 快速退出防护；<br>2. `AgentEvaluationService` 深度集成 Redis 持久化，启动时自动恢复历史样本与运行计数；<br>3. `PathPlanningAgent` 引入 `balanceStageCapacities` 动态阶段容量自适应均衡器；<br>4. 全链路打通 `query` 自然语言参数，前端 HUD 增加自定义诉求输入框并联动意图分发器实体提取。 |
| **36** | **客服智能体意图识别误判、意图吞没与字符子串碰撞陷阱（购物车内含'购'碰撞/泛匹配设置劫持技术问答/宽泛数字越权误杀/会话切换竞态）** | 1. `isCrossUserAttempt` 粗暴判断 `contains("用户") && (... || "2" || "3")` 导致“Vue 3怎么学”、“新用户推荐2门课”被误判拦截；<br>2. `isPageNavigatorIntent` 泛匹配“设置/提问”导致学生询问“Spring Boot设置超时时间”被强行弹出个人设置卡片；<br>3. `isCartViewIntent` 在 `isCoursePurchaseIntent` 前拦截且使用 `contains("购")`，而“购物车”词汇本身自带“购”字，造成“查看我的购物车”因误命中加购动词而恒为 `false`；同时“加到购物车”因在加购前执行被提前吞没；<br>4. 学霸天梯单字匹配“积分”误杀“微积分/定积分”算法问答；<br>5. 前端在 AI 思考推演中允许切换/归档会话导致消息流串流错乱；<br>6. 卡片正则非贪婪单行在嵌套 JSON 首个 `}` 提前截断且定时器在组件卸载时泄漏。 | 1. 越权拦截采用精准正则提取 `(查|看|调取)...(用户|学员|账号)...[数字]`，放行“我的/本人”上下文；<br>2. 页面穿梭严格限定必须包含导航导向动词（`去|前往|跳转|打开|进入|导航|回`）；<br>3. `isCartViewIntent` 在检测动词前先剥离“购物车”关键词，并在加购意图中显式纳入“加购物车/加入购物车”，使“查看”与“加购”泾渭分明；<br>4. 天梯榜排除微积分/定积分数学专有名词并移除无边界的“积分”单字匹配；<br>5. 前端会话切换/新建/归档/删除前增加 `if (isTyping.value) return` 守卫，杜绝竞态覆盖；<br>6. 正则升级为跨行贪婪适配 `/\[AGENT_ACTION_CARD:([\s\S]+?)\]/`，并在 `onBeforeUnmount` 销毁定时器。 |
| **39** | **前端课程中心 404/登录自销毁重载/管理端与登录页 IAIC 统一重塑（路由未注册与API脱落/登录成功自动注销/管理端视觉风格孤立）** | 1. 课程中心路由 `/classList` 虽有页面文件但未在 `base.js` 中注册，且 `class.js` 漏导 `getFreeClassList` 导致打包中断并报 404；<br>2. `permission.js` 在访问 `/login` 时对已登录用户直接调用 `logout()`，结合 `LoginPass.vue` 不当的 `location.reload()` 导致登录瞬间被自清退回登录页；<br>3. 登录页与管理端沿用旧版黑灰 SaaS 风格，与主站 IAIC 科技蓝完全割裂。 | 1. 在 `base.js` 中补齐 `/classList` 路由并补全 `getFreeClassList` API，同时加入 `PUBLIC_PATHS` 免登访问；<br>2. 修复 `permission.js`：已登录用户访问 `/login` 自动重定向至主页，杜绝误注销；修复 `LoginPass.vue` 采用 `router.push` 平滑跳转；<br>3. 登录页 1:1 融入 IAIC 蓝图网格粒子与玻璃拟态卡片，业务管理端全量导入 IAIC 科技蓝 Token 与 Deep Navy 侧边栏及 Cyan 激活高光。 |
| **38** | **全系统 105 项深度缺陷审计与 IAIC 逆向设计移植（IDOR 越权/0元购绕过/优惠券自动穿透/HikariCP 长事务/IAIC 1:1 前端重塑）** | 1. 访客未登录时 `EduUtils.currentUserId()` 伪装为管理员 1L 导致全站 IDOR 越权；<br>2. 下单接口信任请求体 `isInternalSeckill` 导致 0 元购任意付费课程；<br>3. 传入任意 couponId 在 else 分支自动新建直接抵扣实现无限次套利；<br>4. 面试提交作答在 `@Transactional` 中包含外部大模型 HTTP 调用霸占数据库连接；<br>5. 题库抽取在空题目时 `.get(0)` 越界与重复出题；<br>6. 全局异常处理在参数校验异常时缺少字段判断引发 NPE；<br>7. 原门户主页为传统白底，缺少顶尖产学研科技感。 | 1. `currentUserId()` 访客返回 null，增删改显式调用 `requireCurrentUserId()` 阻断未登录越权；<br>2. `placeOrder` 强制从课程 Snapshot 取真实价格，彻底剥离外部秒杀参数；<br>3. 严格校验用户持有未使用的真实优惠券，删除自动新建穿透逻辑；<br>4. 剥离面试事务边界，原子落库后外部异步调用大模型与向量检定；<br>5. 题库抽取增加空列表安全回退至标准问题；<br>6. 校验异常增加多重非空与字段错误判空；<br>7. 逆向 `https://iaic-universities.cqaip.cn/home` 提取全套 `:root` Design Tokens，1:1 重构导航扫描线、Hero 粒子脉冲发光背景、悬浮公告栏、直角科技卡片与 Deep Navy 页脚。 |
| **37** | **课程分类显示全 0 门课程与用户端个人中心静态图片裂图破损（vite-svg-loader 组件化陷阱 / 实体多字段脱节 / 缓存自愈）** | 1. `vite-svg-loader` 默认将 import 的 `.svg` 解析为 Vue 组件函数对象 `{ render: render }`，直接赋给 `<img :src="defaultCover">` 或在 `@error` 兜底事件中赋值给 `e.target.src` 时，会被转为字符串 `"[object Object]"`，导致浏览器请求 404 永久裂图；<br>2. 后端教育学情接口 `EduLearningRecord` 原先仅注入 `cover` 字段，而前端 `ClassCards.vue` 读的是 `courseCoverUrl`；交易微服务订单与购物车项原先只注入 `cover` 字段，而前端 `OrderCards.vue` 读的是 `coverUrl`，导致数据层和展示层字段脱节回退到破损默认图；<br>3. 课程分类表 `tb_category` 自身无课程总数字段，`EducationService` 分类列表接口原先未关联 `tb_course` 聚合统计，返回前端课程数为 null，前端旧缓存（`count: 0`）造成全站显示“0门专业课”。 | 1. 全站所有用作 `<img>` 标签图片源的 SVG 导入统一附加 `?url` 查询后缀（如 `import defaultCover from '@/assets/images/courses/default-cover.svg?url'`），强制 Vite 产出静态资源 URL 路径；<br>2. 后端 `EducationService` 与 `TradeService` 全面对齐注入 `cover`, `coverUrl`, `courseCoverUrl` 三向兼容字段，前端组件采用 `:src="data.courseCoverUrl || data.coverUrl || data.cover || defaultCover"` 链式兜底；<br>3. 后端 `EducationService.listCategories` 与 `category(id)` 增加 `courseMapper.selectMaps` 聚合统计，动态注入真实的 `courseCount` / `courseNum` / `courses`；前端 `readHomeCache` 增加自愈逻辑，对历史存量为 0 的分类自动按字典补全，彻底消灭“0门专业课”。 |
| **39** | **前端课程中心 404/登录自销毁重载/管理端与登录页 IAIC 统一重塑（路由未注册与API脱落/登录成功自动注销/管理端视觉风格孤立）** | 1. 课程中心路由 `/classList` 虽有页面文件但未在 `base.js` 中注册，且 `class.js` 漏导 `getFreeClassList` 导致打包中断并报 404；<br>2. `permission.js` 在访问 `/login` 时对已登录用户直接调用 `logout()`，结合 `LoginPass.vue` 不当的 `location.reload()` 导致登录瞬间被自清退回登录页；<br>3. 登录页与管理端沿用旧版黑灰 SaaS 风格，与主站 IAIC 科技蓝完全割裂。 | 1. 在 `base.js` 中补齐 `/classList` 路由并补全 `getFreeClassList` API，同时加入 `PUBLIC_PATHS` 免登访问；<br>2. 修复 `permission.js`：已登录用户访问 `/login` 自动重定向至主页，杜绝误注销；修复 `LoginPass.vue` 采用 `router.push` 平滑跳转；<br>3. 登录页 1:1 融入 IAIC 蓝图网格粒子与玻璃拟态卡片，业务管理端全量导入 IAIC 科技蓝 Token 与 Deep Navy 侧边栏及 Cyan 激活高光。 |
| **38** | **个人中心「我的课程」最近学习展示异常（分页包装对象误当单课实体 / catalogId为空脱落章节索引 / 兜底课程联动）** | 1. `myClass.vue` 的 `getLearningData` 直接将接口 `/lessons/now` 返回的包装对象 `{ total: 5, list: [...] }` 赋给 `learningData`。而卡片组件 `<ClassCards :data="learningData" type="1"/>` 期待的是单门课程对象，直接访问 `data.courseName`、`data.sections` 均为 `undefined`，导致课程名空白、已学节数显示为 `已学习: / `、正在学习显示为 `正在学习: 第节`，封面回退至默认图；<br>2. 用户无学习记录时接口返回 `{ total: 0, list: [] }`，原先判定 `learningData != null` 依然命中，导致空白镂空卡片强行显示；<br>3. 后端 `learningView` 在 `catalogId == null` 时未查询该课程第一章节信息，导致章节名与序号脱落。 | 1. `myClass.vue` 健壮提取目标单课对象（优先 `res.data.list[0]`），校验 `courseId` 与 `courseName`；仅当存在真实课程时才渲染「最近学习」卡片；若无学习记录但存在「全部课程」，自动选取第一门课程作为待学习展示；<br>2. `ClassCards.vue` 模板全面增强防御性兜底：标题使用 `courseName || name || title`，节数使用 `learnedSections ?? completedLessons ?? 0` 与 `sections || totalLessons || 0`，章节使用 `data.latestSectionName ? 第...节 : 课程导学`，彻底消除 `已学习: / ` 与 `第节` 缺陷；<br>3. 后端 `EducationService.learningView` 增加在 `catalogId == null` 时从 `catalogMapper` 自动查询第一小节的自愈逻辑，确保 `latestSectionName` 与 `latestSectionIndex` 恒不为 null。 |
...
| **44** | **Tomcat RequestFacade 容器回收与 `@Async` 日志 Feign 拦截器并发崩溃** | 1. 业务主线程 HTTP 请求结束后，Tomcat 会异步回收 `RequestFacade` 对象以复用对象池；<br>2. 若主线程中触发了 `@Async` 异步操作（如 `AsyncLogService.saveSysLog` 异步保存操作日志）或者子线程保留了 `RequestContextHolder` 引用，当子线程内部通过 Feign 调用 `remoteLogService` 时，`FeignRequestInterceptor` 尝试调用 `httpServletRequest.getHeaderNames()` 会触发 Tomcat 底层的 `IllegalStateException: The request object has been recycled and is no longer associated with this facade` 并导致微服务内部报错；<br>3. `ServletUtils.getRequest()` 虽做判空但未探测 Request 底层有效性。 | 1. 在 `ServletUtils.getRequest()` 中增加探测机制，若底层被回收直接捕获异常并返回 `null`；在 `getHeader` 与 `getHeaders` 中包裹 `try-catch` 防御回收异常；<br>2. 在 `FeignRequestInterceptor` 中主动探测 `httpServletRequest.getMethod()`，若对象已回收则优雅降级走 `SecurityContextHolder` 兜底；<br>3. 在 `AsyncLogService.saveSysLog` 异步方法入口显式调用 `RequestContextHolder.resetRequestAttributes()`，主动解绑已被回收的主线程上下文，彻底杜绝日志记录异常。 |
| **45** | **RocketMQ 延时关单消费者在已支付/已取消订单上的重试风暴与死信队列污染** | 1. 用户下单后发送 15 分钟延时关单 MQ 消息；<br>2. 若用户在 15 分钟内完成支付或手动取消，当延时消息到达 `TradeOrderTimeoutConsumer` 时，内部调用 `tradeInternalService.cancelOrder(orderId)`；<br>3. 原 `TradeInternalController` 若直接调用抛出业务异常或返回失败，会导致消费端认为处理失败并抛出异常，触发 RocketMQ 默认 16 次重试机制，白白浪费网络与 CPU I/O，并在重试耗尽后沦为死信队列（DLQ）脏数据。 | 1. 在 `TradeInternalController.cancelOrder` 增加幂等防护逻辑：提前查询订单，若订单不存在或 `orderStatus != 0`（已支付或已取消），直接返回 HTTP 200 成功响应并附带提示“无需重复取消”；<br>2. 在 `TradeOrderTimeoutConsumer` 中增强结果校验与日志监控，确保合法业务状态流转平滑 ACK，彻底消灭重试风暴与死信堆积。 |
| **46** | **退款跨服务逆向履约未指定 `userId` 导致误撤销管理员权限（Feign 显式用户契约）** | 1. 在管理后台执行退款审批或退款逆向履约时，原先 `TradeOrderServiceImpl` 调用 `educationService.revokeEnrollment(courseId)`，仅传递了课程 ID；<br>2. 原接口内部根据当前上下文 `SecurityUtils.getUserId()` 获取用户，此时上下文中的用户是正在审核的「管理员（admin/1）」，导致系统错把管理员账号从课程名单中撤除，而真实购买该课程的学员账号反而未被撤销；<br>3. 类似情况在退款/退课、代客下单等管理员代客业务中极易发生越权与身份错乱。 | 1. 扩展 `RemoteEducationService` 契约，新增支持显式传入 `userId` 与 `SecurityConstants.INNER` 的 `enroll(courseId, userId, source)` 和 `revokeEnrollment(courseId, userId, source)` 接口，并提供 Java 8 `default` 方法实现向前兼容；<br>2. `TradeOrderServiceImpl` 履约及逆向履约显式从 `order.getUserId()` 提取真实学员 ID 传递给教育微服务；<br>3. 教育微服务 `EducationInternalController` 针对 `userId` 参数进行严格校验，优先以参数中的 `userId` 为准，杜绝管理员身份串号。 |
| **47** | **客服微服务网关路由前缀与过期活跃会话自动清理的复合安全边界（Gateway 路由 `/customer/**` 映射而非 `/cs/**` / 消息表与会话表秒级时间差防护 / 归档隔离）** | 1. Spring Cloud Gateway 中 `share-customer` 的路径映射是 `/customer/**`（`StripPrefix=1`），而非通用的 `/cs/**`（`/cs/**` 在网关被分配给教育微服务 `share-education`），若请求错误发送至 `/cs/session` 会被教育服务拦截并报 404；<br>2. 会话生命周期清理若仅判断 `cs_session.updated_at`，当高频发消息与会话更新产生微秒级偏差时存在误删风险；且已归档会话（`status = 4`）若无状态隔离会被定时任务连带清理。 | 1. 客服会话相关所有外部请求与管理端接口统一走 `/customer/session/**` 与 `/customer/admin/sessions/**`；<br>2. 会话清理 SQL 必须采用复合强校验：限定未软删除（`del_flag = 0`）且未归档（`status != 4`），同时结合 `updated_at <= cutoffTime` 与 `NOT EXISTS (SELECT 1 FROM cs_message m WHERE m.session_id = s.id AND m.create_time > cutoffTime)`，确保 2 天内无任何后续对话；<br>3. 采用分片批处理（`batchSize = 200`）执行软删除，并在学员访问列表时加入动态自愈排查。 |
| **48** | **客服语义意图向量锚点与动作处理器校验缺失导致求学/推荐问题被购课卡片错误短路截断，跳过大模型与多智能体推演** | 1. `COURSE_PURCHASE` 意图向量锚点语料中混入“我想购买这门推荐的付费专业课”（含“推荐”和“课”），导致用户提问“帮我推荐几门课程”时计算出的余弦相似度偏向购课意图；<br>2. `CustomerActionFactory` 在命中意图后直接调用 `handler.handle(...)`，完全跳过了对应 Handler 的 `supports` 规则校验；<br>3. `CoursePurchaseActionHandler` 在提取课程关键词为空时盲选数据库首门课并生成购课卡片，使得 `actionReply` 非空，将 `isAgentDeliberation` 设为 true，从而直接短路跳过了后续第 2 步（导学多智能体推演）与第 3 步（大模型客服答疑）。 | 1. 净化 `SemanticIntentRouter` 锚点：`COURSE_PURCHASE` 纯化为购买/下单/结算专属，`PATH_PLANNING` 显式纳入“帮我推荐几门课程”等语料；<br>2. 在 `CustomerActionFactory` 意图派发前增加 `handler.supports(...)` 防御校验，向量命中但规则不符时坚决放行；<br>3. `CoursePurchaseActionHandler` 增加前置 `supports` 审计，排除无明确购买动词的纯推荐咨询；<br>4. 保证求学推荐意图稳定进入 `orchestrateAgentRecommend` 多智能体协同推演与大模型规划。 |
